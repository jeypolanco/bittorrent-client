...
class PieceAssembler(object):
    ...
    def set_connected_peers(self, peer_list, connected_peers, reactor):
            ...
            if peer.is_connected():
                ...
                for peer in self.peer_list:
                    ...
                    if len(self.connected_peers) < 5:
                        peer = PeerListener(self.reactor, peer, self.metainfo, self)
                        if peer.is_connected():
    ...
    def get_missing(self):
        ...
        for piece in self.missing_pieces:
            if piece in self.transit:
                ...
                print len(self.free_peers)
                for peer in self.free_peers:
                    print peer.pieces_list
                    print peer
                break
#peer.has_piece(piece)
#                    if peer.has_piece(piece):
#                        peer.get_piece(piece)
...
class TorrentClient(object):
    def __init__(self, metainfo, tracker_list, reactor, piece_assembler):
        self.reactor = reactor
...
#                print peer_list
                self.init_peers(peer_list, metainfo, piece_assembler)


    def init_peers(self, peer_list, metainfo, piece_assembler):
        for peer in peer_list:
            # Change this back to 25 when you finish debugging 
            if len(self.connected_peers) < 5:
                peer = PeerListener(self.reactor, peer, metainfo.metainfo, piece_assembler)
                if peer.is_connected():
                    self.connected_peers.append(peer)
                else:
                    continue
        piece_assembler.set_connected_peers(peer_list, self.connected_peers, self.reactor)
...
class PeerListener(object):
    def __init__(self, reactor, peer_addr, metainfo, piece_assembler):
        ...
        self.id_dict = {'bitfield': 5, 'have': 4, 'keep alive': 0}
        ...
        try:
            ...
            self.piece_assembler.register(self)
    ...
    def generate_msg(self):
        if self.choked:
            ...
            for block in self.piece_blocks:
                if not piece_blocks[block]:
                    if piece_blocks[block] == len(piece_blocks) - 1 and self.last_block:
                        block_length = struct.pack("i", self.last_block)
                        block_offset = struct.pack("i", self.block_request_size * block)
                        message_len = struct.pack("i", 13)
                        return block_length + request_id + block_offset + message_len
                    else:
                        block_length = struct.pack("i", self.block_request_size)
                        block_offset = struct.pack("i", self.block_request_size * block)
                        message_len = struct.pack("i", 13)
                        return block_length + request_id + block_offset + message_len
                else:
...
class Reactor(object):                                                        
    ...
    def loop(self):
        ...
        for w in ws:
            ...
            w.on_write()
        self.piece_assembler.update_connected_peers()
...
def main():
    ...
    piece_assembler = PieceAssembler(t_parser.get_metainfo().metainfo)
    reactor = Reactor(piece_assembler)
    t_client = TorrentClient(t_parser.get_metainfo(), t_parser.get_metainfo().get_trackerList(), reactor, piece_assembler)
    while True:
        reactor.loop()
